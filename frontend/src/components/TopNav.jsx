...
export default function TopNav({ mode, setMode, onLibrary, lang, setLang, t = {} }) {
  const [dark, setDark] = useState(() => {
    return localStorage.getItem("nier-theme") === "dark";
  });

  useEffect(() => {
    document.documentElement.setAttribute("data-theme", dark ? "dark" : "light");
    localStorage.setItem("nier-theme", dark ? "dark" : "light");
  }, [dark]);

  const buttonStyle = {
    padding: "10px 14px",
    border: "1px solid var(--nier-border, #3a3a3a)",
    background: "var(--nier-bg-btn, #dcd6c4)",
  ...
  const activeStyle = {
    ...buttonStyle,
    background: "var(--nier-btn-active-bg, #2b2b2b)",
    color: "var(--nier-btn-active-text, #f5f1e8)",
  };
  ...
  return (
    <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center" }}>
      <button
        className={`nier-btn${mode === "character" ? " nier-btn-active" : ""}`}
      ...
      <button
        className={`nier-btn${mode === "lore" ? " nier-btn-active" : ""}`}
      ...
      <button
        className={`nier-btn${mode === "config" ? " nier-btn-active" : ""}`}
      ...
      {/* Library */}
      <button className="nier-btn" style={buttonStyle} onClick={onLibrary} title="Open saved cards">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ marginRight: 6, verticalAlign: "middle" }}>
      ...
      {/* Language toggle */}
      <div style={{ display: "flex", border: "1px solid var(--nier-border, #3a3a3a)" }}>
        {[{code: "en", label: "EN"}, {code: "ja", label: "日本語"}, {code: "zh", label: "中文"}].map(({code, label}) => (
          <button
            key={code}
            className={`nier-btn${lang === code ? " nier-btn-active" : ""}`}
            ...
            style={{
              ...
              border: "none",
              borderRight: code !== "zh" ? "1px solid var(--nier-border, #3a3a3a)" : "none",
            }}
          >
            {label}
          </button>
