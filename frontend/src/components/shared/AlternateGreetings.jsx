/**
 * AlternateGreetings.jsx — Alternate greeting manager
 * =====================================================
 * Manages the list of alternate first messages on a character card.
 * Renders below the first_mes field when it is the selected field.
 *
 * PROPS
 * -----
 * greetings        Array of greeting strings
 * onChange(arr)    Called with the updated array on any change
 * t                Translation strings
 * cardId           DB id of the card (for per-greeting regeneration)
 *
 * FEATURES
 * --------
 * - Add blank greeting
 * - Edit greeting text inline
 * - Delete greeting
 * - Regenerate individual greeting via POST /api/card/:id/field/regen
 *   with fieldName "alternate_greetings[n]"
 *
 * HOW TO EXTEND
 * -------------
 * - Add reorder: implement drag-and-drop or up/down arrow buttons
 *   that swap elements in the greetings array.
 * - Add per-greeting export: add a copy button that puts the greeting
...
export default function AlternateGreetings({ greetings = [], onChange, t = {}, cardId }) {
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      {greetings.length === 0 && (
        <div style={{ fontSize: 12, color: "var(--signal-text-dim)", marginBottom: 8, textAlign: "center" }}>
      ...
      {greetings.map((g, idx) => (
        <div key={idx} style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            <span style={{ fontSize: 10, letterSpacing: "0.1em", color: "var(--signal-text-dim)" }}>
          ...
          <textarea
            ...
            style={{
              ...
              padding: 10,
              border: "1px solid var(--signal-border)",
              background: busyIdx === idx ? "var(--signal-bg-mid)" : "var(--signal-bg-field)",
              color: busyIdx === idx ? "var(--signal-text-dim)" : "var(--signal-text)",
              resize: "vertical",
              fontFamily: "inherit",
              fontSize: 14,
              boxSizing: "border-box",
