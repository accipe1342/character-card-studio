/**
 * PreviewPanel.jsx — Card image and export panel
 * ================================================
 * Displays the card image upload area, a live text preview of the
 * card fields, and the Export as PNG button.
 *
 * PROPS
 * -----
 * imageFile        File object for the uploaded image (or null)
 * imageUrl         Object/proxy URL for the image preview (or null)
 * onImageChange(file, url)  Called when user uploads a new image
 * card             The current card object
 * viewMode         "natural" | "structured" — which fields to preview
 * t                Translation strings
 *
 * EXPORT BEHAVIOUR
 * ----------------
 * Clicking Export calls POST /api/card/:id/export/png which returns
 * a PNG with the SillyTavern metadata embedded as a tEXt chunk
 * (key: "chara", value: base64-encoded V2 card JSON). This is the
 * format SillyTavern expects for character card imports.
 *
 * The "Include fields in image" checkbox adds the structured profile
 * as a text overlay on the image before embedding.
 *
 * The Natural/Structured toggle (only visible when include fields is
 * checked) controls which view is overlaid on the image.
 *
 * HOW TO EXTEND
 * -------------
 * - Change the export format: the PNG embedding is done server-side
 *   in backend/app.py (api_export_png). Edit there to change the
 *   metadata format.
 * - Add more export formats: add buttons and handlers that call
 *   different export endpoints.
 * - Change the preview layout: edit the card preview section inside
...
export default function PreviewPanel({ imageFile, imageUrl, onImageChange, card, t = {} }) {
    ...
    try {
      ...
      if (includeFields) {
        if (viewMode === "natural") {
          ...
          const order = ["age","sex","gender","ethnicity","race","pronouns","sexual_attraction",
  ...
  return (
    <section className="signal-panel signal-screen-panel" style={{ border: "1px solid var(--signal-border)", background: "var(--signal-bg-panel)", padding: 14, position: "relative" }}>
      <h2 className="signal-heading" style={{ marginTop: 0 }}>{t.preview || "Preview"}</h2>

      <label style={{
        display: "grid", placeItems: "center", minHeight: 48,
        border: "1px dashed #3a3a3a", marginBottom: 12,
        cursor: "pointer", background: "var(--signal-bg-field)",
      }}>
      ...
      <div
        style={{
          ...
          border: "1px solid var(--signal-border)", background: "var(--signal-bg-mid)",
      ...
      >
        {imageUrl ? (
          ...
          <div style={{ width: "100%", height: "100%", display: "grid", placeItems: "center", color: "var(--signal-text-dim)" }}>
            {t.noImage || "No image uploaded yet"}
          </div>
        )}
      </div>

      <div style={{ color: "var(--signal-text-dim)", fontSize: 13, marginBottom: 12 }}>
        {imageFile ? `${t.selected || "Selected"}: ${imageFile.name}` : (t.uploadHint || "Upload art, portrait, or cover image.")}
      </div>
      ...
      {exportError && (
        <div style={{
          ...
          <div>
            <div style={{ fontWeight: 700, marginBottom: 4 }}>{t.exportFailed || "Export Failed"}</div>
            <div>{exportError}</div>
          </div>
        </div>
      )}
      ...
      {card && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "center" }}>
          ...
          <label style={{
            ...
            cursor: "pointer", fontSize: 13, color: "var(--signal-text)", userSelect: "none",
          ...
          {includeFields && (
            <div style={{ display: "flex", gap: 6 }}>
              {[["natural", t.natural || "NATURAL"], ["structured", t.structured || "STRUCTURED"]].map(([m, label]) => (
                <button key={m}
                  className={viewMode === m ? "signal-btn signal-btn-active" : "signal-btn"}
          ...
          {/* Export button */}
          <button className="signal-btn" onClick={handleExportPng} disabled={exporting}
            style={{
              ...btnStyle,
              width: "100%",
              background: "var(--signal-btn-active-bg)", color: "var(--signal-text-inv)",
