/**
 * ConfigPage.jsx — Settings and API key management
 * ==================================================
 * The Config tab. Allows the user to set API keys, choose default
 * provider/model, and edit custom prompt templates.
 *
 * SECTIONS
 * --------
 * API Keys    Input fields for NANOGPT_API_KEY, OPENROUTER_API_KEY,
 *             LOCAL_OPENAI_BASE_URL, LOCAL_MODEL, LOCAL_API_KEY.
 *             Saved via POST /api/config which writes to .env.
 *
 * Prompt Templates  Text areas for the custom system prompts used
 *                   during character and lore generation.
 *                   Saved via POST /api/config/prompts.
 *
 * PROPS
 * -----
 ...
 * HOW TO EXTEND
 * -------------
 * - Add a new setting: add an input field, include the key in the
 *   POST /api/config body, and handle it in app.py's api_save_config.
 * - Add a new prompt template: add a textarea and a new key to
 *   prompt_templates.py's default dict.
 */
...
export default function ConfigPage({ t = {}, showToast }) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto" }}>
      <h1 className="signal-title signal-heading" style={{ textAlign: "center", marginBottom: 4 }}>
        Config
      </h1>
      <p style={{ textAlign: "center", color: "var(--signal-text-dim)", marginBottom: 24, letterSpacing: "0.04em" }}>
      ...
      <section className="signal-panel signal-screen-panel" style={{
        border: "1px solid var(--signal-border)", background: "var(--signal-bg-panel)", padding: 20, marginBottom: 20,
      }}>
        <h2 className="signal-heading" style={{ margin: "0 0 16px", fontSize: 15 }}>Generation Parameters</h2>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          {/* Temperature */}
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em" }}>
                Temperature
              </label>
              <span style={{ fontSize: 13, fontWeight: 700, color: "var(--signal-text)" }}>
                {templates.temperature?.toFixed(1) ?? "0.7"}
              </span>
            </div>
            <input
              type="range" min="0" max="1" step="0.1"
              value={templates.temperature ?? 0.7}
              onChange={e => setTemplates(p => ({ ...p, temperature: parseFloat(e.target.value) }))}
              style={{ width: "100%", accentColor: "var(--signal-text)" }}
            />
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "var(--signal-text-dim)", marginTop: 4 }}>
          ...
          {/* Max Tokens */}
          <div>
            <label style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", display: "block", marginBottom: 6 }}>
              Max Tokens
            </label>
            <select
              value={templates.max_tokens ?? 4096}
              onChange={e => setTemplates(p => ({ ...p, max_tokens: parseInt(e.target.value) }))}
              style={{
            ...
            >
              <option value={1024}>1024 — fast / cheap</option>
              <option value={2048}>2048 — balanced</option>
              <option value={4096}>4096 — default</option>
              <option value={8192}>8192 — long responses</option>
            </select>
            <div style={{ fontSize: 10, color: "var(--signal-text-dim)", marginTop: 4 }}>
              Higher = longer output, more tokens used
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="signal-panel signal-screen-panel" style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h2 className="signal-heading" style={{ margin: 0, fontSize: 15 }}>Prompt Templates</h2>
          <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13 }}>
            <span style={{
              width: 16, height: 16, border: "1.5px solid var(--signal-border)",
              background: templates.use_templates ? "var(--signal-btn-active-bg)" : "var(--signal-bg-field)",
              display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
            }}>
              {templates.use_templates && (
                <svg width="10" height="8" viewBox="0 0 10 8" fill="none">
                  <path d="M1 3.5L3.5 6.5L9 1" stroke="var(--signal-text-inv)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              )}
            </span>
        ...
        {/* Tab bar */}
        <div style={{ display: "flex", borderBottom: "1px solid var(--signal-border)", marginBottom: 14, gap: 0 }}>
          {TABS.map(t => (
            <button key={t.key}
              className={activeTab === t.key ? "signal-btn signal-btn-active" : "signal-btn"}
              onClick={() => setActiveTab(t.key)}
              style={{ padding: "8px 14px", fontSize: 12, fontWeight: 700, border: "none",
                borderBottom: activeTab === t.key ? "2px solid var(--signal-text)" : "2px solid transparent",
        ...
        {/* Hint + variable chips */}
        <div style={{ fontSize: 11, color: "var(--signal-text-dim)", marginBottom: 8 }}>
          {tab?.hint}
        </div>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
          {tab?.vars.map(v => (
            <button key={v} className="signal-btn" onClick={() => insertVar(v)}
              style={{ padding: "3px 10px", fontSize: 11, fontFamily: "monospace", border: "1px solid var(--signal-border)", cursor: "pointer" }}>
              {v}
            </button>
          ))}
          <button className="signal-btn" onClick={() => setTemplates(p => ({ ...p, [tab.templateKey]: tab.defaultVal }))}
            style={{ padding: "3px 10px", fontSize: 11, border: "1px solid var(--signal-border)", cursor: "pointer", marginLeft: "auto", color: "var(--signal-text-dim)" }}>
            Reset to default
          </button>
        </div>
        ...
        <div style={{ marginTop: 12, display: "flex", justifyContent: "flex-end" }}>
          <button className="signal-btn" onClick={saveTemplates} disabled={saving}
            style={{ ...btnStyle, opacity: saving ? 0.5 : 1, background: "var(--signal-btn-active-bg)", color: "var(--signal-text-inv)" }}>
      ...
      {/* ── API Keys ─────────────────────────────────────── */}
      <section className="signal-panel signal-screen-panel" style={{
        border: "1px solid var(--signal-border)", background: "var(--signal-bg-panel)", padding: 20,
      }}>
        <h2 className="signal-heading" style={{ margin: "0 0 16px", fontSize: 15 }}>API Keys</h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          {[
            ...
            <label key={key} style={{ fontSize: 12, color: "var(--signal-text-dim)", display: "flex", flexDirection: "column", gap: 4 }}>
              {label}
                ...
                {isKey && (
                  <button onClick={() => setShowKeys(p => ({ ...p, [key]: !p[key] }))}
                    style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)",
                      background: "none", border: "none", cursor: "pointer", color: "var(--signal-text-dim)", fontSize: 13 }}>
                    {showKeys[key] ? "◎" : "○"}
                  </button>
                )}
              </div>
        ...
        <div style={{ marginTop: 14, display: "flex", justifyContent: "flex-end" }}>
          <button className="signal-btn" onClick={saveEnv} disabled={savingEnv}
            style={{ ...btnStyle, opacity: savingEnv ? 0.5 : 1, background: "var(--signal-btn-active-bg)", color: "var(--signal-text-inv)" }}>
